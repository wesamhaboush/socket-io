...
socket.on('connection', function(client){ 
	console.log('client connection made: ' + client);	
	// Success!  Now listen to messages to be received
	client.on('message',function(event){ 
		console.log('Received request from client!',event);
                requestResultsOnTheFly(client);
	});
...
function requestResultsOnTheFly(socket){
        ...
        http.request(OPTIONS, function(inres) {
          ...
          inres.on('data', function (chunk) {
              ...
              if(requestsCompleted == REQUEST_COUNT) {
                  socket.emit('message', { completed: requestsCompleted } );
                  socket.end();
              }
          }); 
        }).end();
